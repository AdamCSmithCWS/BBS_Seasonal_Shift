...

{
...

	{
		...
		log(lambda[k]) <- obs[strat[k],obser[k]] + eta*firstyr[k] + strata[strat[k]] + yeareffect[strat[k],year[k]] + yy[strat[k],year[k]] + seasoneffect[latbelt[k],decade[k],season[k]]  + noise[k]
...
##### seasonal GAM by decade
	for(k in 1:nknots_season)
	  ...
	  for(l in 1:nlat_belts)
	  {
	    # Computation of GAM components
	    
	    B_season[l,k] ~ dnorm(0,tau_B_season)
	  ...
	  beta_season[l,1,k] <- B_season[l,k]#+beta_season_p[1,k]
	      ...
	      
	    {
	      ...
	      beta_season[l,i,k] ~ dnorm(beta_season[l,i-1,k],taubeta_season) #
	        ...
	        
	      {
	        ...
	        season_part[l,i,k,t] <- beta_season[l,i,k]*(season_basis[t,k])
	    ...
	    
	  {
	      ...
	      
	    {
	      for(l in 1:nlat_belts)
	      {
	      seasoneffect[l,i,t] <- sum(season_part[l,i,1:nknots_season,t])
	      }#l
	    }#t
